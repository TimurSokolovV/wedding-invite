...
function setProgramMap(a) {
  ($btn = $j(this)), ($modal = $j("#program-map"));
  var e = $modal.find("#program-map-event-id"),
    t = $modal.find("#program-map-reason"),
    o = $j("#program-map-wrapper");
  o.show(), t.hide(), null != mapProgram && mapProgram.destroy();
  var r = [];
  ...
  if (r)
    if (
      ((mapProgram = new ymaps.Map("program-map-wrapper", {
      ...
      e.find("option").each(function (a, e) {
          ...
          m.push(
            new ymaps.Placemark(
              t.data("coords").split(","),
              {
                ...
                clusterCaption:
                  t.data("date-when") + " в " + t.data("time-when"),
              },
        ...
        mapProgram.setBounds(p.getBounds(), { checkZoomRange: !0 });
    }
  else o.hide(), t.text("Координаты события не обнаружены"), t.show();
}
$j("#program p.addr span.value, #section-program p.addr span.value").on(
  "click",
  setProgramMap
),
  $j("#program-map #program-map-event-id").on("change", setProgramMap);
